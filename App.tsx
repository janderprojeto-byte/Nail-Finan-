...
import { Transaction, ExpenseType, MonthlyExpense, Revenue, Withdrawal, ProLaboreFrequency, DistributionConfig, ProfitCycle } from './types';
...
import { Plus, User, Briefcase, Sparkles, PieChart, Coins, Table, LogOut, LayoutDashboard, Quote } from 'lucide-react';
...
const App: React.FC = () => {
  const [user, setUser] = useState<string | null>(() => {
    return localStorage.getItem('glow_auth_session');
  });
  ...
  const [transactions, setTransactions] = useState<Transaction[]>(() => {
    ...
    return INITIAL_EXAMPLE_TRANSACTIONS;
  });
  ...
  const handleLogin = (userName: string) => {
    setUser(userName);
    localStorage.setItem('glow_auth_session', userName);
  };

  const handleLogout = () => {
    if (confirm('Deseja sair da sua conta? Seus dados continuarão salvos com segurança.')) {
      setUser(null);
      localStorage.removeItem('glow_auth_session');
    }
  };
    ...
    <div className="min-h-screen bg-[#FDFCFD] text-gray-900 pb-20 md:pb-0 md:pl-64 relative overflow-hidden">
      <aside className="fixed left-0 top-0 h-full w-64 bg-white/90 backdrop-blur-md border-r border-gray-100 hidden md:flex flex-col z-40">
        ...
        <div className="p-4 mb-4">
            <button onClick={handleLogout} className="w-full flex items-center gap-2 px-4 py-3 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-2xl transition-all font-bold text-[11px]">
        ...
        <header className="mb-8 space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="space-y-1">
              <h2 className="text-3xl font-display font-bold text-gray-800">Olá, {user?.split(' ')[0]} ✨</h2>
