...
import { Sparkles, Lock, User, UserPlus, LogIn, Eye, EyeOff } from 'lucide-react';
...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isLogin) {
      const savedUser = localStorage.getItem(`auth_user_${username.toLowerCase().trim()}`);
      if (savedUser) {
      ...
      const key = `auth_user_${username.toLowerCase().trim()}`;
      ...
      <div className="w-full max-w-md relative z-10 animate-in fade-in zoom-in duration-500">
        ...
        <div className="bg-white/80 backdrop-blur-xl p-8 rounded-[48px] shadow-2xl border border-white/50">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  placeholder="Seu nome ou stúdio"
          ...
          <p className="mt-8 text-center text-[10px] text-gray-400 font-medium">
            Seus dados são armazenados apenas no seu navegador. Segurança local e privada.
          </p>
        </div>
      </div>
    </div>
