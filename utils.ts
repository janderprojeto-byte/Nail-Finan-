...
export const calculateProfitReserve = (
  allRevenues: Revenue[],
  currentMonth: number,
  currentYear: number,
  profitPercent: number,
  cycleMonths: ProfitCycle
) => {
  const startMonth = Math.floor(currentMonth / cycleMonths) * cycleMonths;
  let accumulatedProfit = 0;

  for (let m = startMonth; m <= currentMonth; m++) {
    const mRevenues = allRevenues
      .filter(r => {
        const d = new Date(r.date + 'T12:00:00');
        return d.getMonth() === m && d.getFullYear() === currentYear;
      })
      .reduce((acc, curr) => acc + curr.amount, 0);
    accumulatedProfit += (mRevenues * (profitPercent / 100));
  }

  const isWithdrawalMonth = (currentMonth + 1) % cycleMonths === 0;
  
  let cycleLabel = '';
  if (cycleMonths === 1) cycleLabel = 'Ciclo Mensal';
  else if (cycleMonths === 3) cycleLabel = `Ciclo Trimestral (${getMonthName(startMonth)} - ${getMonthName(startMonth + 2)})`;
  else if (cycleMonths === 6) cycleLabel = `Ciclo Semestral (${getMonthName(startMonth)} - ${getMonthName(startMonth + 5)})`;
  else cycleLabel = `Ciclo Anual (Janeiro - Dezembro)`;

  return {
    accumulatedProfit,
    cycleLabel,
    isWithdrawalMonth,
    monthsCompleted: currentMonth - startMonth + 1
  };
};
...
/**
 * Gera previsões de pró-labore baseadas no faturamento REAL ACUMULADO.
 * Se o faturamento do dia 23 ainda não aconteceu, ele não afetará a sugestão do dia 11.
 */
export const generateProLaborePredictions = (
  ...
  // Data de início (ajuste ou primeiro ganho)
  ...
  // Define os dias de intervalo baseado na frequência
  let intervalDays = 1;
  ...
  // Itera pelos dias do mês para gerar as previsões
  for (let day = 1; day <= daysInMonth; day += intervalDays) {
    ...
    // 1. Calcular Faturamento Real Acumulado ATÉ ESTA DATA
    ...
    // 2. Calcular o quanto desse faturamento "pertence" ao Pró-labore
    // Se for modo PERCENT, é a porcentagem direta.
    // Se for modo FIXED, calculamos a proporção do faturamento atual em relação ao necessário.
    const maxProLaboreAvailable = revenueUntilNow * (proLaborePercent / 100);

    // 3. Subtrair o que já foi retirado no mês ATÉ AGORA
    ...
    // 4. Calcular a "fatia" ideal para esta data específica
    const divisor = getFrequencyDivisor(frequency);
    const idealSlice = proLaboreMode === 'PERCENT' 
      ? (revenueUntilNow * (proLaborePercent / 100)) / divisor // Aproximação da fatia
      : fixedProLaboreValue / divisor;

    // A sugestão nunca pode ser maior que o que tem no caixa (faturamento acumulado)
    // nem maior que a fatia ideal planejada.
    let suggestedAmount = Math.min(idealSlice, remainingAvailable);

    // Evitar sugerir valores negativos ou irrelevantes (menos de 1 real)
    if (suggestedAmount < 1) continue;

    // Verificar se já existe uma retirada para este exato dia para não duplicar
    ...
    // Se for mensal, encerra no primeiro match
